...
const DropdownBtn = (params: any) => {
  ...
  const deleteMatiere = async () => {
    try {
      const response = await axios.delete(`/api/admin/news/${matiere.id}`);
      if (response.status === 200) {
        toast.success("La nouvelle a été supprimé avec succès");
        location.reload();
      }
    } catch (error) {
      ...
      <>
        <div className="flex">
          <div className="relative flex items-center transition-colors ease-in justify-start text-slate-900 border rounded-md cursor-pointer">
            ...
            <div className="absolute bg-white hover:bg-slate-100 right-0 w-8 flex justify-center items-center rounded-md  h-full">
              <DropdownMenu>
                <DropdownMenuTrigger>
                  <ChevronDown />
                </DropdownMenuTrigger>
      ...
      <>
        <div className="flex">
          <div className="relative flex items-center transition-colors ease-in justify-start text-white bg-slate-900 border rounded-md cursor-pointer">
            ...
            <div className="absolute bg-slate-900 hover:bg-slate-800 right-0 w-8 flex justify-center items-center rounded-md  h-full">
              <DropdownMenu>
                <DropdownMenuTrigger>
                  <ChevronDown />
                </DropdownMenuTrigger>
