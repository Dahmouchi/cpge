...
export async function GET(
  ...
  const student = await prisma.video.findUnique({
    where: {
      id: id,
    },
    include:{
...
export async function DELETE(
    ...
    try {
        const matiere = await prisma.video.delete({
            where: {
                id: id,
            },
        });
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Step 1: Update the post fields (title, description, etc.)
    ...
    // Return the updated post and file
