...
const PostsComponents = ({ post, reload }: any) => {
  ...
  const deleteContent = async () => {
    try {
      const response = await axios.delete(`/api/video/${post.id}`);
  ...
  const updatePost = async (formData: FormData) => {
    try {
      ...
      const response = await axios.put(`/api/video/${post.id}`, formData);
      if (response.status === 200) {
  ...
  <CardHeader className="flex flex-col justify-between h-full">
    <div>
    <video className="w-full h-48 object-cover border-b-4 border-emerald-700" controls>
        <source
      ...
      <CardTitle className="truncate w-full mt-3">{post.title}</CardTitle>
        ...
        <DialogContent className="max-w-4xl sm:max-w-2xl md:max-w-3xl lg:max-w-3xl">
          ...
          >
            ...
            />
              ...
              <div className="overflow-y-auto h-40 my-3">
