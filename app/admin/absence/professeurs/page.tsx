...
const Classes = () => {
  ...
  const getInfos = async () => {
    ...
    try {
      const fetchedEtudiants = await axios.get(`/api/profClass/${selectedClass}`);
  ...
  const submitAbsence = async () => {
    const selectedStudents = filteredEtudiants.filter((etudiant) => etudiant.selected); 
    const absencePromises = selectedStudents.map((student) =>
      axios.post('/api/profClass', {
  ...
  return (
    <div className="container mx-auto py-2 space-y-4">
      <div className="sticky top-0  z-10 space-y-2 pb-2">
          ...
          <div>
            <h1 className="text-2xl font-semibold mb-4">La gestion d'absence pour les professurs</h1>
        ...
        {controle && 
         <div className="flex flex-col space-y-4">
           <div className="flex flex-row justify-between items-center">  
             <h1 className="text-lg font-semibold">Les professeurs de la classe {selectedClass}</h1>
